...
Paste messy notes. Get better, tighter prose that still sounds like you. That should not be controversial. Apparently we needed a tool anyway.

Youish turns rough drafts, notes, and rants into clear writing without saving every stray line just because it has personality. It is not AI trying to borrow a soul. It is an editor making the point land while protecting the one already in the draft.
...
```bash
gh skill install RegionallyFamous/youish skills/youish --agent codex --scope user --pin v0.3.12
```

```bash
curl -fsSL https://raw.githubusercontent.com/RegionallyFamous/youish/v0.3.12/install.sh | YOUISH_REF=v0.3.12 bash
```

For a deterministic release-asset install that pulls the published skill ZIP and verifies it against `SHA256SUMS`:

```bash
curl -fsSL https://raw.githubusercontent.com/RegionallyFamous/youish/v0.3.12/install.sh | YOUISH_REF=v0.3.12 YOUISH_SOURCE=release-zip bash
```
