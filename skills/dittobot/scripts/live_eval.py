...
def user_prompt(case: Case) -> str:
    ...
    if case.ordered_terms:
        lines.append(f"- Keep these ideas in order: {', '.join(case.ordered_terms)}")
    if case.exact_substrings:
        lines.append(f"- Preserve these exact substrings: {', '.join(case.exact_substrings)}")
    if case.line_prefixes:
        lines.append(f"- Preserve lines starting with: {', '.join(case.line_prefixes)}")
    if case.exact_words is not None:
        lines.append(f"- Use exactly {case.exact_words} words.")
    if case.max_words is not None:
        lines.append(f"- Use at most {case.max_words} words.")
    if case.exact_paragraphs is not None:
        lines.append(f"- Use exactly {case.exact_paragraphs} paragraphs.")
    if case.max_paragraphs is not None:
        lines.append(f"- Use at most {case.max_paragraphs} paragraphs.")
    if case.starts_with:
        lines.append(f"- Start with: {case.starts_with}")
    if case.ends_with:
        lines.append(f"- End with: {case.ends_with}")
    if case.preserve_uncertainty:
        lines.append("- Preserve uncertainty; do not make the claim sound more certain.")
    if case.no_dash:
    ...
    if case.forbid_wrappers:
        lines.append("- Do not include a rewrite label or wrapper.")
    if case.forbid_clarifying:
        lines.append("- Do not ask a clarifying question.")
    if case.forbid_bullets:
        lines.append("- Do not use bullets.")
    if case.max_question_marks is not None:
        lines.append(f"- Use at most {case.max_question_marks} question marks.")
    if case.forbid_artifacts:
        lines.append(f"- Remove these draft artifacts: {', '.join(case.forbid_artifacts)}")
