...
def main() -> int:
    ...
    if (ROOT / "README.md").exists():
        if not SPEC_SKILL_ROOT.exists():
            fail("skills/dittobot package mirror is missing", errors)
        else:
            for rel in PACKAGE_FILES:
                source = ROOT / rel
                mirror = SPEC_SKILL_ROOT / rel
                if not mirror.exists():
                    fail(f"skills/dittobot/{rel} is missing from package mirror", errors)
                elif source.exists() and digest(source) != digest(mirror):
                    fail(f"skills/dittobot/{rel} is out of sync with root package file", errors)
